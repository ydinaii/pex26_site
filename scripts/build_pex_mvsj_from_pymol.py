...
def build_overview_snapshot():
    ...
    # No .color() call — matches the proven pattern in build_pex_mvsj.py, which
    # leaves ball_and_stick/spacefill at Mol*'s default (element-based) coloring
    # rather than specifying a color theme explicitly.
    ligands = structure.component(selector="ligand")
    ligands.representation(type="ball_and_stick")

    ions = structure.component(selector="ion")
    ions.representation(type="spacefill")
...
def build_inset_snapshot():
    ...
    site.representation(type="ball_and_stick")  # default coloring, no explicit color theme
